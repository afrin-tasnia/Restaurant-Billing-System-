...
void addOrder() {
    if (total_customers >= MAX_CUSTOMERS) {
        printf("Sorry, maximum number of customers reached!\n");
        return;
    }
    ...
    while (more == 'y' || more == 'Y') {
        ...
        printf("\nEnter item numbers separated by spaces (1-%d): ", MAX_ITEMS);
        char input[100];
        fgets(input, sizeof(input), stdin);
        ...
        while (token != NULL) {
            ...
            if (item_num >= 1 && item_num <= MAX_ITEMS) {
                char prompt[50];
                sprintf(prompt, "Enter quantity for %s: ", items[item_num - 1]);
                int qty = getNumberInput(prompt);

                if (qty > 0) {
                    orders[current_customer].quantities[item_num - 1] += qty;
                    printf("Added %d %s\n", qty, items[item_num - 1]);
                } else {
                    printf("Quantity must be positive! Item not added.\n");
                }
            } else {
                printf("Invalid item number: %d (skipped)\n", item_num);
            }
...
void viewBill() {
    ...
    for (int i = 0; i < MAX_ITEMS; i++) {
        int qty = orders[index].quantities[i];
        if (qty > 0) {
